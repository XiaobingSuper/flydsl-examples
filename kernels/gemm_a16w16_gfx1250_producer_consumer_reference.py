...
"""Historical producer/consumer learning reference for gfx1250.

This is the complete pre-convergence implementation preserved for studying
dedicated TDM producer waves, named barriers, and the circular LDS pipeline.
Production dispatch lives in ``gemm_a16w16_gfx1250.py`` and must not import
...
separate A/B TDM producer waves, a configurable circular LDS pipeline, and
runtime M/N bounds; optimized all-compute routes pad unsupported extents in
their private wrapper.
"""
...
_PRODUCER_EXACT_CONFIGS = {
    (32, 64, 7168): (1, 1, 4, 2, 1),
}
_ALL_COMPUTE_8W_SHAPE_CONFIGS = {
    (128, 2048, 4096): dict(reg_n=1, num_buffers=4),
    (512, 2048, 7168): dict(reg_n=4, num_buffers=3),
    (2048, 1024, 7168): dict(reg_n=4, num_buffers=3),
}
...
def _select_production_route(
    ...
    if (m, n, k) in _ALL_COMPUTE_8W_SHAPE_CONFIGS:
...
def _create_all_compute_module(
    ...
    *,
    ...
    waves_per_eu = 2 if num_waves == 8 else 1
    ...
    def kernel(
        ...
        arena = fx.SharedAllocator(static=False)
        arena.allocate(arena_bytes)
        base_bf16 = fx.recast_iter(fx.BFloat16, arena.base_ptr)
    ...
    *,
    ...
    if not _skip_validation:
        ...
        if a.dtype != torch.bfloat16 or b.dtype != torch.bfloat16:
            raise ValueError("a and b must both have bfloat16 dtype")
    else:
        m, k = a.shape
        n = b.shape[0]
...
def _select_config(
    ...
    if all(value is None for value in (reg_m, reg_n, reg_k, waves_m, waves_n)):
        exact = _PRODUCER_EXACT_CONFIGS.get((m, n, k))
    ...
    *,
    ...
    if route == _ROUTE_ALL_COMPUTE_8W:
        return _gemm_all_compute(
            ...
            **_ALL_COMPUTE_8W_CONFIG,
            **_ALL_COMPUTE_8W_SHAPE_CONFIGS[(m, n, k)],
        )
